...
const Cart = ({item, isDarkMode}) => {
    ...
    const handleClickVideoPlay = (id, ip_address) => {
        setLoading(true)
        axios.get(`${ip}/api/face/video/${ip_address}/${id}`)
            .then(res => {
                const {data} = res;
                console.log("data = ", data);
                setLoading(false);
                window.open(`${ip}/anhor_camera_video/${id}.h264`, '_blank', 'noopener,noreferrer');
            })
            .catch(err => {
                                ...
                                <p className="card_longTitle">
                                    {
                                        item && item.old === 'age_0_10'
                                            ? <Tooltip title={t('Yosh bola')} color={'cyan'}
                                                       className={`${isDarkMode && 'darkModeColor'}`}>{t('Yosh bola')}</Tooltip>
                            ...
                            <div>
                                ...
                                <p className="card_longTitle">{item.args.KOZOYNAK === 1 ?
                                    <Tooltip title={t('Ko’rishni_tuzatish')} color={'cyan'}
                                             className={`${isDarkMode && 'darkModeColor'}`}>{t('Ko’rishni_tuzatish')}</Tooltip> :
                                    item.args.KOZOYNAK === 14 ? <Tooltip title={t('Quyoshdan_himoya')} color={'cyan'}
                                                                         className={`${isDarkMode && 'darkModeColor'}`}>{t('Quyoshdan_himoya')}</Tooltip> :
                                        <Tooltip title={t('Kozoynaksiz')} color={'cyan'}
                                                 className={`${isDarkMode && 'darkModeColor'}`}>{t('Kozoynaksiz')}</Tooltip>}
                                </p>
                            </div>
